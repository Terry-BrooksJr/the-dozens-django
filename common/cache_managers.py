...
class CategoryCacheManager(BaseCacheManager):
    ...
    def set_category_name_mapping(self, category_key: str, name: str) -> None:
        ...
        cache_keys = self.get_cache_keys()
        # name_prefix + key → stores the name (used by get_category_name_by_key)
        cache_key_for_name_lookup = f"{cache_keys['name_prefix']}{category_key}"
        # key_prefix + name → stores the key (used by get_category_key_by_name)
        cache_key_for_key_lookup = f"{cache_keys['key_prefix']}{name.lower()}"
