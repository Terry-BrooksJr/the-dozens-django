...
class CachedResponseMixin(GenericAPIView):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.bulk_select_related: Union[List[str], None] = getattr(
            self, "bulk_select_related", None
        )
        self.bulk_prefetch_related: Union[List[str], None] = getattr(
        ...
        if (
            ...
            raise AttributeError(
                "Viewset must have a 'bulk_select_related' attribute defined. If No select_related is needed, set it to 'None'"
            )
        if (
            not self.bulk_prefetch_related and self.bulk_prefetch_related is not None
        ) or self.bulk_prefetch_related == []:
            raise AttributeError(
                "Viewset must have a 'bulk_prefetch_related' attribute defined. If No prefetch_related is needed, set it to 'None'"
            )
    ...
    def get_cached_response(self, cache_key: str) -> Union[Response, None]:
        ...
        if cached_data is not None:
            ...
            metrics.increment_cache(
                model=self.primary_model.__name__, cache_type="miss"
            )
            return None
    ...
    def get_cached_bulk_data(
        ...
        if cached_data is not None:
            ...
            metrics.increment_cache(
                model=self.primary_model.__name__, cache_type="bulk_hit"
            )
        ...
        metrics.increment_cache(
            model=self.primary_model.__name__, cache_type="bulk_miss"
        )
...
class OptimizedListSerializer(serializers.ListSerializer):
    ...
    def to_representation(self, data):
        ...
        if hasattr(data, "select_related") and hasattr(
            self.child, "select_related_fields"
        ):
            if self.child.select_related_fields:
                data = data.select_related(*self.child.select_related_fields)

        if hasattr(data, "prefetch_related") and hasattr(
            self.child, "prefetch_related_fields"
        ):
